...
> 📖 **准备面试？** 直接阅读 [面试自述（5 个技术故事 + 追问速答）](docs/interview-self-report.md) 或 [系统流程文档（820 行，14 章节）](docs/system-flow-diagrams.md)
...
面试中最有技术深度的模块。**手写的工作流引擎，不是调用现成框架。**
...
| **单体架构** | Spring Boot 单体 | 1 人项目拆微服务是负优化，面试会被追问服务发现、分布式事务、数据一致性 |
...
| **文档** | PRD · 系统流程文档 · 后端架构文档 · Android 架构文档 · 面试自述 · 刷题模块 API 需求 · [架构决策记录 (ADR)](docs/adr/) · [CHANGELOG](docs/CHANGELOG.md) |
...
以下 3 个决策面试中最可能被追问，提前准备好完整的 Trade-off 论证。
...
> 📖 面试追问深度拆解 → [面试自述 § DAG 引擎](docs/interview-self-report.md)
...
> 📖 分布式限流设计细节 → [面试自述 § 多租户](docs/interview-self-report.md)
...
> 📖 AI 出题 + SymPy 验证完整拆解 → [面试自述 § 语义缓存 + RRF](docs/interview-self-report.md)
...
### 已知局限（Phase 2 前不会修，但面试可能会问）

| 局限 | 当前应对 | 面试话术 |
|------|---------|---------|
| SymPy 无法验证几何证明题（覆盖率 67%） | 走合理性检查 + LLM 二次自检，解析末尾标注 `"AI 生成，经基础校验"` | "符号验证是目前数学题自动验证的最佳折衷——覆盖了最高频的计算和方程类题目（占总题量 60-70%），剩余的用规则引擎 + LLM 投票兜底。" |
...
│   ├── interview-self-report.md      #   面试自述（5 个技术故事 + 追问速答）
│   └── backend-task-practice-module.md # 刷题模块后端开发任务
│
└── ai/                               # 📋 产品设计
